...
def parse_arguments(argv, no_defaults=False):
    ...
    # The multiworld initialization wants this, not sure that it needs it though
    ret.multi = multiargs.multi
    ...
    # Get the Namespace object so things don't break
    # Just initialize the Namespace
    #ret = parser.parse_args()

    # Copy this because it seems to matter for now
    # TODO: Remove this, handle it in yaml parsing!!!
    # if multiargs.multi:
    #     for player in range(1, multiargs.multi + 1):
    #         playerargs = parse_arguments(shlex.split(getattr(ret, f"p{player}")), True)
    #
    #         game = getattr(ret, "game") if getattr(playerargs, "game") is None else getattr(playerargs, "game")
    #         if player == 1:
    #             setattr(ret, "game", {1: game})
    #         else:
    #             getattr(ret, "game")[player] = game
    ...
    # These should all be moved now, and removing this *should*
    # not have any side effects... leaving it here for now
    # if ret.timer == "none":
    #     ret.timer = False
